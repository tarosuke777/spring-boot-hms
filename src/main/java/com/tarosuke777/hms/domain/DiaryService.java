...
public class DiaryService {
    ...
    public List<DiaryForm> getDiaryList(String orderBy, String sort) {
        return diaryMapper.findMany(orderBy, sort).stream().map(entity -> {
    ...
    public void registerDiary(DiaryForm form) {
        diaryMapper.insertOne(form.getDiaryDate(), form.getTodoPlan(), form.getTodoActual(),
                form.getFunPlan(), form.getFunActual(), form.getCommentPlan(),
                form.getCommentActual());
    }

    @Transactional
    public void updateDiary(DiaryForm form) {
        diaryMapper.updateOne(form.getDiaryId(), form.getDiaryDate(), form.getTodoPlan(),
                form.getTodoActual(), form.getFunPlan(), form.getFunActual(), form.getCommentPlan(),
                form.getCommentActual());
    }
