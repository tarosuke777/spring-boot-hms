...
public class CastControllerTest {
    ...
    void getList_ShouldReturnCastList() throws Exception {
        // Given
        LoginUser loginUser = (LoginUser) TestSecurityContextHolder.getContext().getAuthentication()
                .getPrincipal();
        Integer currentUserId = loginUser.getId();
        List<CastForm> expectedCastList = castRepository.findByCreatedBy(currentUserId).stream()
                .map(castMapper::toForm).toList();
    ...
    private ResultActions performGetDetailRequest(int id) throws Exception {
        return mockMvc.perform(
                get(DETAIL_ENDPOINT, id).accept(MediaType.TEXT_HTML).characterEncoding("UTF-8"))
                .andDo(print());
    }
